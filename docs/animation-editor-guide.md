...
- 模组 id：`free_camera_api_tripod`，根包 `cn.anecansaitin.free_camera_api_tripod`
...
```
F6 ──► CameraEditorScreen ─┐
                          ├─► EditorContext ──┬─► CameraEditorModel   编辑操作（增删键、拖路径点…）
按键 / 面板交互 ───────────┘                  ├─► CameraPlayer        时间推进 + 姿态求值
...
## 5. 编辑器 GUI（`core.editor`）

### 5.1 屏幕家族

| 屏幕 | 说明 |
| --- | --- |
| `CameraEditorScreen` | 主编辑器：顶部菜单（文件/编辑/视图/播放/帮助）+ 可拖拽停靠的面板布局 |
...
- 交互反馈：`notify(Component)` 状态行提示、`confirm(...)` 二次确认弹窗（由屏幕统一绘制与派发）
...
- `EditorConfig`：客户端配置，含布局串 `layout.dock`、下排高度、折叠面板、视口提示收起、深色模式、`dev.test_keys`
...
6. **辅助界面**：`FileBrowserScreen` / `StorageBrowserScreen`（面包屑与列表）、`WorldViewScreen`、`PathHandleDrag`、`ViewportTakeover`
